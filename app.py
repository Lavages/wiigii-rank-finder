...
import threading # Import for background threading of data preload
import sys # Import for sys.stdout.flush() in preload_wca_data

from specialist import specialist_bp, preload_wca_data # Import preload_wca_data
from completionist import get_completionists, preload_completionist_data # <--- Import preload_completionist_data

app = Flask(__name__)

# Register blueprint for specialist routes
app.register_blueprint(specialist_bp, url_prefix='/api')
CORS(app)  # Enable CORS for all origins

# --- Data Preloading Logic ---
# This block ensures that the WCA data is loaded when the application starts.
# For production environments (like Render), a WSGI server like Gunicorn
# will typically load the application code.
# Starting the preload in a separate thread allows the Flask app to
# start serving requests while data loading happens in the background.
# The `DATA_LOADING_LOCK` in specialist.py prevents redundant loading.

# Check if running in a production-like environment (e.g., not during local development)
if os.environ.get('FLASK_ENV') != 'development':
    print("Detected non-development environment. Starting WCA data preloading in background.", file=sys.stdout)
    sys.stdout.flush()
    # Start the preload in a separate daemon thread.
    # `daemon=True` means the thread will exit when the main program exits.
    preload_specialist_thread = threading.Thread(target=preload_wca_data, daemon=True)
    preload_specialist_thread.start()

    # Start Completionist data preload in another daemon thread
    preload_completionist_thread = threading.Thread(target=preload_completionist_data, daemon=True)
    preload_completionist_thread.start()
else:
    # For local development, you might want direct call for immediate feedback.
    # This will block Flask startup until data is loaded.
    print("Detected development environment. Preloading WCA data directly (may block startup).", file=sys.stdout)
    sys.stdout.flush()
    preload_wca_data() # For specialists
    preload_completionist_data() # For completionists

# Base URLs for WCA APIs (these are used by other parts of your app, not by specialist_bp after preloading)
...
OFFICIAL_EVENTS = [
    ...
    "clock","minx","pyram","skewb","sq1","444bf","555bf","333mbf" # Corrected "443bf" to "444bf"
]
...
# Route to serve completionist.html
...
try:
    ...
    def completionists(category): # <--- Added 'category' parameter here
        data = get_completionists()
        return jsonify(data)
except ImportError:
    print("Warning: 'completionist' module not found. '/api/completionists' routes will not work.", file=sys.stderr)
    @app.route("/api/completionists", defaults={"category": "all"})
    @app.route("/api/completionists/<category>")
    def completionists_placeholder(category): # <--- Added 'category' parameter here for the placeholder too
        return jsonify({"error": "Completionist functionality not available (module not found)."}), 500


# ------------------- Specialist Routes (HTML Page Serving) -------------------
@app.route("/specialist")
def serve_specialist_page():
    """Serve the HTML page for specialists."""
    return send_from_directory(os.path.dirname(os.path.abspath(__file__)), "specialist.html")

# ------------------- General API Helper Functions (used by other routes, not specialist_bp) -------------------
    ...
    for i in range(max_retries):
        try:
            ...
            if i < max_retries - 1:
                ...
                print(f"Retrying {url} in {sleep_time:.2f}s due to error: {e}", file=sys.stderr)
                time.sleep(sleep_time)
            else:
                print(f"Failed to fetch {url} after {max_retries} retries: {e}", file=sys.stderr)
                raise
    return None
...
def fetch_person(wca_id):
    ...
    try:
        ...
        print(f"⚠️ Error fetching person {wca_id}: {e}", file=sys.stderr)
        return None
...
def get_rankings(wca_id):
    ...
    print(f"Fetching rankings for {wca_id} from {rankings_url}", file=sys.stdout)
...
def get_global_rankings(region, type_param, event):
    ...
    print(f"Fetching global rankings from {rankings_url}", file=sys.stdout)
    ...
    try:
        ...
        status = getattr(e.response, "status_code", 500)
        print(f"Error fetching global rankings: {e}", file=sys.stderr)
